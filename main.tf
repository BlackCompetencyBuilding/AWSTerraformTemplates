...
# the resource to create public subnets in Management VPC
 ...
 resource "aws_subnet" "mngmt_public_subnet" {
  ...
  tags = "${merge(var.tags, var.public_subnet_tags, map("Name", format("Mangement-subnet-public")))}"
...
# the resource to create private subnets in Management VPC
 ...
 resource "aws_subnet" "mngmt_private_subnet" {
  ...
  tags = "${merge(var.tags, var.private_subnet_tags, map("Name", format("Mangement-subnet-private")))}"
}
...
resource "aws_vpc" "non_prod_vpc" {
  cidr_block  = "${var.non_prod_workload_vpc_cidr}"
  instance_tenancy = "${var.instance_tenancy}"
  enable_dns_hostnames = "${var.enable_dns_hostnames}"
  enable_dns_support = "${var.enable_dns_support}"
...
# the resource to create public subnets in Non Prod VPC
 ...
 resource "aws_subnet" "non_prod_public_subnet" { 
  count = "${length(var.public_subnets_non_prod)}"
  vpc_id   = "${aws_vpc.non_prod_vpc.id}"
  map_public_ip_on_launch = true
  cidr_block = "${var.public_subnets_non_prod[count.index]}"
  tags = "${merge(var.tags, var.public_subnet_tags, map("Name", format("NonProd-subnet-public-%s", element(var.public_subnets_non_prod, count.index))))}"

}

# the resource to create private subnets in Non Prod VPC

 resource "aws_subnet" "non_prod_private_subnet" {
  count = "${length(var.private_subnets_non_prod)}"
  vpc_id   = "${aws_vpc.non_prod_vpc.id}"
  cidr_block = "${var.private_subnets_non_prod[count.index]}"
  tags = "${merge(var.tags, var.private_subnet_tags, map("Name", format("NonProd-subnet-private-%s", element(var.private_subnets_non_prod, count.index))))}"
}

# the resource to create an internet gateway for Non Prod VPC

resource "aws_internet_gateway" "non_prod_igw" {
  #count = "${length(var.public_subnets_non_prod) > 0 ? 1 : 0}"
  vpc_id = "${aws_vpc.non_prod_vpc.id}"
...
resource "aws_route_table" "non_prod_publicrt" {
  #count = "${length(var.public_subnets_non_prod) > 0 ? 1 : 0}"
  vpc_id = "${aws_vpc.non_prod_vpc.id}"
...
resource "aws_route_table" "non_prod_privatert" {
  #count = "${length(var.private_subnets_non_prod) > 0 ? 1 : 0}"
  vpc_id = "${aws_vpc.non_prod_vpc.id}"
...
resource "aws_route" "non_prod_public_routes" {
  #count = "${length(var.public_subnets_non_prod) > 0 ? 1 : 0}"
  route_table_id         = "${aws_route_table.non_prod_publicrt.id}"
  destination_cidr_block = "0.0.0.0/0"
...
resource "aws_route_table_association" "non_prod_public" {
  count = "${length(var.public_subnets_non_prod)}"
  subnet_id      = "${element(aws_subnet.non_prod_public_subnet.*.id, count.index)}"
  route_table_id = "${element(aws_route_table.non_prod_publicrt.*.id, count.index)}"
}

 # Private Route Table Association
resource "aws_route_table_association" "non_prod_private" {
  count = "${length(var.private_subnets_non_prod)}"
  subnet_id      = "${element(aws_subnet.non_prod_private_subnet.*.id, count.index)}"
  route_table_id = "${element(aws_route_table.non_prod_privatert.*.id, count.index)}"
}
...
resource "aws_vpc_peering_connection" "Management-non_prod"
{
...
resource "aws_route" "mngmt2nonprod" {
  route_table_id = "${aws_route_table.mngmt_privatert.id}"
  destination_cidr_block = "${aws_vpc.non_prod_vpc.cidr_block}"
  vpc_peering_connection_id = "${aws_vpc_peering_connection.Management-non_prod.id}"
}
...
resource "aws_route" "nonprod2mngmt" {
  route_table_id = "${aws_route_table.non_prod_privatert.id}"
  destination_cidr_block = "${aws_vpc.mgmt_vpc.cidr_block}"
  vpc_peering_connection_id = "${aws_vpc_peering_connection.Management-non_prod.id}"
}
